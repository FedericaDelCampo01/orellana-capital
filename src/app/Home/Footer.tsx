'use client'

import Image from "next/image";
import logoImage from "../../../public/images/logo.svg";

const Footer = () => {

  return (
    <section className="bg-dark-blue font-sans overflow-hidden w-full px-4 md:pt-24 pb-8">
      <div className="container mx-auto md:max-w-5xl bg-white/[.14] rounded-xl md:rounded-3xl md:mb-16">
        <div className="flex flex-wrap md:flex-nowrap justify-between p-8 md:p-12 md:pb-8">
          <div className="flex flex-col justify-between w-full md:w-1/4 mb-8 md:mb-0">
            <Image
              src={logoImage}
              alt="Logo"
              width={150}
              height={47}
              className=" -ml-4 -mt-1"
            />
            <div className="flex space-x-4 mt-6">
              <a href="https://www.linkedin.com/company/coddit-io" target="_blank" rel="noopener noreferrer" className="text-white hover:text-light-blue">
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" clipRule="evenodd" />
                </svg>
              </a>
              <a href="https://github.com/coddit-io" target="_blank" rel="noopener noreferrer" className="text-white hover:text-light-blue">
                <svg className="w-8 h-8" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" />
                </svg>
              </a>
            </div>
          </div>

          <div className="w-full md:w-1/4 mb-8 md:mb-0">
            <h3 className="text-white text-lg font-medium mb-4">Links</h3>
            <p className="text-white/70 text-sm font-extralight mb-2">
              <a href="/" className="hover:text-light-blue">Home</a>
            </p>
            <p className="text-white/70 text-sm font-extralight mb-2">
              <a href="#about-us" className="hover:text-light-blue">About Us</a>
            </p>
            <p className="text-white/70 text-sm font-extralight mb-2">
              <a href="#services" className="hover:text-light-blue">Services</a>
            </p>
            <p className="text-white/70 text-sm font-extralight mb-2">
              <a href="#why-coddit" className="hover:text-light-blue">Why Coddit</a>
            </p>
            <p className="text-white/70 text-sm font-extralight mb-2">
              <a href="#contact" className="hover:text-light-blue">Contact</a>
            </p>
          </div>

          <div className="w-full md:w-1/4 mb-8 md:mb-0">
            <h3 className="text-white text-lg font-medium mb-4">Services</h3>
            <p className="text-white/70 text-sm font-extralight mb-2">Product Discovery</p>
            <p className="text-white/70 text-sm font-extralight mb-2">Custom Software Development</p>
            <p className="text-white/70 text-sm font-extralight mb-2">AI Development</p>
            <p className="text-white/70 text-sm font-extralight mb-2">App Development</p>
            <p className="text-white/70 text-sm font-extralight mb-2">Web Development</p>
            <p className="text-white/70 text-sm font-extralight mb-2">Staff Augmentation</p>
          </div>
          
            {/* GreenGeeks Seal Code */}
            <div className="flex flex-col w-full md:w-1/4 md:text-right">
              <a 
                href="https://greengeeks.com/" 
                target="_blank"
                rel="noopener noreferrer"
                className="flex justify-end"
              >
                <img
                  src="https://static.greengeeks.com/ggseal/Green_22.png" 
                  alt="GreenGeeks Seal" 
                  className="w-24 h-24 "
                />
              </a>
            </div>
            {/* End GreenGeeks Seal Code */}
      
        </div>
        <div className="container mx-auto px-8 md:px-12 pb-8">
          <hr className="border-t border-white/50 mb-8" />
          <p className="text-white/70 text-sm font-extralight text-left">
            © Copyright 2024 Coddit. All Rights Reserved
          </p>
        </div>
      </div>
    </section>
  )
}

export default Footer;